...
final class ExtensionManager: NSObject, ObservableObject {
    ...
    func presentInstallPanel() {
        ...
        panel.allowedContentTypes = Self.installableContentTypes
        panel.message = "Choose a Firefox/Chrome extension folder, .xpi, or .zip package"
    ...
    /// Install the bundled Hello Zero sample (Firefox-flavored MV3) for a quick smoke test.
    func installBundledDemoExtension() async throws -> InstalledExtension {
        guard let demoURL = Self.bundledDemoExtensionURL() else {
            throw ExtensionManagerError.loadFailed("Bundled demo extension was not found in the app bundle.")
        }
        return try await installExtension(from: demoURL)
    }

    func revealExtensionsFolder() {
        NSWorkspace.shared.open(extensionsRootURL)
    }

    private static var installableContentTypes: [UTType] {
        var types: [UTType] = [.zip, .folder]
        if let xpi = UTType(filenameExtension: "xpi") {
            types.insert(xpi, at: 0)
        }
        return types
    }

    private static func bundledDemoExtensionURL() -> URL? {
        let subdirectories = [
            "WebExtensions/Demo/hello-zero",
            "Demo/hello-zero",
            "hello-zero"
        ]
        for subdirectory in subdirectories {
            if let url = Bundle.main.url(forResource: "manifest", withExtension: "json", subdirectory: subdirectory) {
                return url.deletingLastPathComponent()
            }
        }

        // Development fallback: Examples/ZeroSampleExtension next to the project.
        let candidates = [
            Bundle.main.bundleURL
                .deletingLastPathComponent() // …/Products
                .deletingLastPathComponent() // …/Build
                .deletingLastPathComponent() // …/DerivedData/...
                .appendingPathComponent("Examples/ZeroSampleExtension"),
            URL(fileURLWithPath: #filePath)
                .deletingLastPathComponent() // WebExtensions
    ...
    private func extractPackage(from sourceURL: URL, to destination: URL) async throws {
        ...
        // Firefox .xpi packages are ZIP archives with a different extension.
        if ext == "zip" || ext == "xpi" {
